...
define('PATH_typo3conf', PATH_site.'typo3conf/');	// TYPO3 configuraton path
...
message(`$command`);
	...
	// Fill the DB with the contents of the old database.sql
...
$typo_db = 'typo3_dbupdater_temp';
	...
	// Save the database information to the constants.
	// config_default.php will try to overwrite those, but it can't because constants cannot be overwritten
	...
	function updateDB()	{
		...
		while (list(,$loadedExtConf)=each($GLOBALS['TYPO3_LOADED_EXT']))	{
			if (is_array($loadedExtConf) && $loadedExtConf['ext_tables.sql'])	{
				$tblFileContent.= "\n\n\n\n".t3lib_div::getUrl($loadedExtConf['ext_tables.sql']);
			}
		}

		if ($tblFileContent)	{
			$fileContent = implode(chr(10), $this->getStatementArray($tblFileContent,1,'^CREATE TABLE '));
	...
	function importStatic()	{
		...
		foreach($GLOBALS['TYPO3_LOADED_EXT'] as $loadedExtConf)	{
			if (is_array($loadedExtConf) && $loadedExtConf['ext_tables_static+adt.sql'])	{
				$tblFileContent.= "\n\n\n\n".t3lib_div::getUrl($loadedExtConf['ext_tables_static+adt.sql']);
			}
		}
